...
void draw(){
// Module LeapMotion - Donnees orientation main :
  Controller controller = new Controller();
  Frame frame = controller.frame();
  rect(0, 0, width, height);
  Hand hand = frame.hands().get(0);
  
  double pitch = Math.toDegrees(hand.direction().pitch()); // Conversion radian -> degres.
  tangage = (int) Math.round(pitch);                       // Arrondi a l'entier.
  //println("Pitch :" + tangage);
  
  double roll = Math.toDegrees(hand.palmNormal().roll());
  roulis = (int) Math.round(roll);
  //println("Roll :" + tangage);
  palmTracking();
...
// Update affichages :
  if(!gameover){  
    ...
    textSize(20);
    fill(0,102,133);
    text("Pitch : " + tangage,width/1.1,height/1.1);
    text("Roll : " + roulis,width/1.1,height/1.05);
...
void palmTracking(){
  if(tangage>-150 && tangage<-10){dir.x = 0; dir.y = -1;}
  if(roulis>20 && roulis<150){dir.x = -1; dir.y = 0;}
  if(tangage>35 && tangage<150){dir.x = 0; dir.y = 1;}
  if(roulis>-150 && roulis<-20){dir.x = 1; dir.y = 0;}
}
